...
Original file is located at
    https://colab.research.google.com/drive/176W2e97dbIUgIktSjqBWUOgXEyxX7n8F
...
![alt text](https://github.com/Francososa/ufm-ai-2019/blob/master/assets/charseq.jpeg?raw=true)
"""
...
chars = tuple(set(text))  # separa por letra
int2char = dict(enumerate(chars))  # asigna un numero a cada letra
char2int = {ch: ii for ii, ch in int2char.items()}  # genera un tipo key-value para cada letra un numero 

# encode the text
encoded = np.array([char2int[ch] for ch in text]) # asigan un int para su letra correspondiente a todo el texto
...
one_hot = one_hot_encode(test_seq, 8) #agarra la secuencia y conviertala en una secuencia de 8 caracteres
...
![alt text](https://github.com/Francososa/ufm-ai-2019/blob/master/assets/sequence_batching.png?raw=true)
...
def get_batches(arr, batch_size, seq_length):
    ...
    '''
    #N * M
    tamanio_total_batch = batch_size * seq_length
    
    ## TODO: Get the number of batches we can make
    num_de_batches = len(arr)//tamanio_total_batch
    
    ## TODO: Keep only enough characters to make full batches
        # Basicamente para que haya un numero parejo en cada batch
    arr = arr[:num_de_batches * tamanio_total_batch]
    ...
    for i in range(0, arr.shape[1], seq_length):
        ...
        y = np.zeros_like(x)
        try:
            # toma un valor despues del primer index hasta el seq_length
            y[:, :-1], y[:, -1] = x[:, 1:], arr[:, i+seq_length]
        except IndexError:
            # si quiere un index fuera del rango
            y[:, :-1], y[:, -1] = x[:, 1:], arr[:, 0]
        yield x, y
...
#imprimir los primeros 10 elementos de una secuencia
print('x\n', x[:10, :10])
print('\ny\n', y[:10, :10])
...
![alt text](https://github.com/Francososa/ufm-ai-2019/blob/master/assets/charRNN.png?raw=true)
...
class CharRNN(nn.Module):
    ...
    def __init__(self, tokens, n_hidden=256, n_layers=2,
        ...
        # Layer de Long-Short Term Memory
        self.long_short_term_m = nn.LSTM(len(self.chars), n_hidden, n_layers, 
                            dropout=drop_prob, batch_first=True)
        
        # Layer de Dropout
        self.dropout = nn.Dropout(drop_prob)
        
        # Layer Final, fully-connected output layer 
        self.fullyC = nn.Linear(n_hidden, len(self.chars))
    ...
    def forward(self, x, hidden):
        ...
        r_output, hidden = self.long_short_term_m(x, hidden)
