...
class Autoencoder(nn.Module):
    ...
    def forward(self, a):
        x = F.relu(self.encoder1(a))
        y = F.relu(self.decoder1(x))
...
class ConvAutoencoder(nn.Module):
    ...
    def forward(self, x):
        y = F.relu(self.conv_encoder1(x))
        y = F.relu(self.max_encoder1(y))
        y = F.relu(self.conv_encoder2(y))
        y = F.relu(self.max_encoder2(y))
        y = F.relu(self.conv_decoder1(y))
        y = F.relu(self.conv_decoder2(y))
        y = self.sigmoid(y)
