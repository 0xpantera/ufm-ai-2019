...
class Autoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        dec = F.sigmoid(self.decoder(enc))
        return dec
...
class ConvAutoencoder(nn.Module):
    def __init__(self):
        ...
        ## encoder layers ##
        self.t_conv_enc = nn.Conv2d(1, 16, 2, padding = 1)
        self.max_pool = nn.MaxPool2d(2, 2)
        
        ## decoder layers ##
        ## un kernel de 2 y un stride de 2 van a incrementar las dims por 2
        self.t_conv_dec = nn.ConvTranspose2d(16, 1, 2, stride=2)
    ...
    def forward(self, x):
        ...
        c1enc = F.relu(self.t_conv_enc(x))
        c2enc = self.max_pool(c1enc)
        c1dec = F.sigmoid(self.t_conv_dec(c2enc))
